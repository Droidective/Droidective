...
export const releases: { version: string; date: string; latest?: boolean; html: string }[] = [
  { version: "v3.12.1", date: "Sep 2026", latest: true, html: "<b>A bug-fix release.</b> The Reactotron timeline and the JS Console each held far more than they should have: both capped 128 MB of received data, but a decoded log row costs several times its size on the wire, so the real cost was closer to a gigabyte <i>each</i> — one session was measured climbing from 363 MB to 1.58 GB over three hours and staying there for a day. <b>Both feeds now size their buffers to your Mac</b> (192 MB apiece on a 16 GB machine). Nothing was leaking; the limit was simply far too generous. <b>And a feed you cannot see stops rendering.</b> Whether the app was visible had been inferred from which tab was in front and whether Droidective was frontmost, so an app sitting behind another window with a Reactotron tab open kept redrawing every second; it now asks the window server directly. Events keep arriving throughout — only the drawing waits — and bringing the window back shows everything at once. Hangs are also measurable at last: a report says how long the app was actually gone for rather than “at least 2 seconds”, and records what each feed was holding at the time. Numbers only, and the privacy toggles still apply." },
