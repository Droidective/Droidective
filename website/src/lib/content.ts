...
export const releases: { version: string; date: string; latest?: boolean; html: string }[] = [
  { version: "v3.1.0", date: "Jul 2026", latest: true, html: "<b>Custom commands, reworked</b> — one box for full command lines exactly as you'd type them in a terminal (multi-line supported), <kbd>adb</kbd> lines detected automatically, and each command can open Droidective's Terminal or your Mac's default terminal. The <b>Quick Actions panel is now yours to curate</b>: pin custom commands with <kbd>⌘P</kbd>, hide actions you don't use, and a <code>{bundleId}</code> command with no app selected asks you to pick one instead of erroring. Updates check twice a day and a dismissed update returns as a notification until installed. The welcome tour ends by pressing your new Quick Actions hotkey — confetti included — and the Terminal's tab list now defaults to the top strip." },
