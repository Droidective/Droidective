...
export const releases: { version: string; date: string; latest?: boolean; html: string }[] = [
  { version: "v3.12.3", date: "Sep 2026", latest: true, html: "<b>Bug-fix release</b> — <b>the Mac's microphone could never be switched on for a recording</b>. Turning it on reported that microphone access was off and offered to open System Settings ▸ Privacy &amp; Security ▸ Microphone, where Droidective was not listed — so there was nothing there to switch on. The app was signed without the permission that lets it ask at all, so macOS refused before you were ever prompted, which is why no permission dialog appeared either. It now asks the first time you turn the microphone on, and there is nothing to undo first: the old refusal was never recorded against the app. Recording the device's own audio was never affected. Plus <b>three fixes from the update framework</b> — two security fixes in how an update is unpacked and installed, and a third that restores window focus for apps running in the background without a Dock icon, the state Droidective is in with “Keep running in the background” on, where an update window could end up behind everything else. And <b>Droidective is now built and checked against macOS 27</b>: it already ran there, so this is the round of source changes Apple's new compiler asked for, along with one in the mirror's video path where the system moved a property the mirror reads for every frame. Nothing behaves differently." },
  { version: "v3.12.2", date: "Sep 2026", html: "<b>Bug-fix release</b> — nine fixes, most of them things the app was getting wrong quietly. <b>Clear Cache spun for thirty seconds and then said it failed</b>: on some Android images that command never returns, so it waited out the full timeout; it is now bounded everywhere it appears. <b>A part-finished APK pull left a file that looked finished</b> — pulling an app that ships as several split files could stop halfway and leave the first one at the path you chose, which looks like the app and fails to install days later; a failed pull now takes back what it wrote, and the progress bar counts every file instead of watching one. <b>Converting an App Bundle could produce an APK that cannot be installed</b> — without your own signing key, bundletool falls back to Android Studio's debug key and silently produces an unsigned APK when the Mac has never had Android Studio on it; the result now says so. <b>“Check for Updates” says what it is doing</b> rather than going grey while an update downloads, and <b>the Apps list can be resized</b>. On the measurement side: <b>a hang report no longer counts time the Mac spent asleep</b> — the duration added in v3.12.1 was read from a clock that keeps running through sleep, so a closed lid read as a multi-minute hang — and Droidective <b>can now say which part of it is holding memory</b> rather than only describing the two streaming feeds. On <b>Windows and Linux</b>: a screen left open at quit no longer strands a polling loop, data from one device can no longer land on another's screen (which on Developer Settings meant a toggle could write to the wrong device), and destructive actions ask with a dialog like the Mac." },
  { version: "v3.12.1", date: "Sep 2026", html: "<b>Pin a tab</b> — right-click a tab ▸ Pin Tab and it sits at the front of its pane and stays there, surviving <i>Close Other Tabs</i>, with the × replaced by a pin so the tabs you keep open all day are out of reach of a stray click. It is not a lock (⌘W still closes one), and a pinned tab stays pinned when you move it to the other pane or drag it into its own window. <b>Send Text's snippets now appear in the Quick Actions panel</b>, under the field — the same recency-ranked list, ↓/↑ to walk it and ⏎ to insert, with <code>{clipboard}</code> and <code>{ip}</code> filling in as usual. Plus the fix for long Reactotron and JS Console sessions turning into an unresponsive app: <b>both feeds now size their buffers to your Mac</b> rather than to a fixed 128 MB of received data, which — once decoded — cost closer to a gigabyte <i>each</i>; one session was measured climbing from 363 MB to 1.58 GB over three hours and staying there for a day. Nothing was leaking; the limit was far too generous. <b>And a feed you cannot see stops rendering</b>: visibility had been inferred from which tab was in front and whether Droidective was frontmost, so an app behind another window with a Reactotron tab open kept redrawing every second. It now asks the window server directly — events keep arriving throughout, only the drawing waits. Hangs are measurable at last, too: a report says how long the app was actually gone for rather than “at least 2 seconds”." },
  { version: "v3.12.0", date: "Sep 2026", html: "<b>Any tab can become its own window</b> — <b>drag a tab out of the app and it becomes a window</b>: put it on another display, beside your editor, or drop it on a second Droidective window to move it in there. Menus do the same thing (right-click a tab, or Tab ▸ Move Tab to New Window, ⌃⌘N), and a torn-off window <b>keeps the device its tab was pointed at</b> rather than picking a free one. <b>And a moved tab keeps working.</b> The Terminal takes its running shells and scrollback; a <b>screen recording keeps recording</b>, so starting it in one window and stopping it in another gives one continuous file; a <b>live mirror keeps streaming</b> with the same session and encoder — no reconnect, no black second — and the Mirror Wall moves all six at once. <b>Unsaved screenshot markup survives</b> with its undo history, a <b>decompiled APK stays decompiled</b> instead of re-running jadx for minutes, and the video editor keeps its edits and the transcoded copy it built to play them. Reactotron, the JS Console, APK Studio, Logcat, iOS Logs, crashes, API Testing, the browsed folder, the Apps list and anything half-typed all come along too. Performance and Network Speed deliberately do not: their sampler stops with the view, and a graph with a silent gap in it is worse than being told. Plus dragging a tab to the empty part of its own tab strip now works, where it used to do nothing at all." },
  { version: "v3.11.0", date: "Sep 2026", html: "<b>Drop files onto a mirror, and pair by QR</b> — <b>drag an app package onto the mirror and it installs on that device</b>; anything else — a photo, a PDF, a folder — copies to <code>/sdcard/Download</code> and is handed to the device’s media library, so a dropped screenshot turns up in the Gallery rather than only in Files. The mirror <b>says what will happen before you let go</b>, and every mirror takes drops — the tab, a popped-out window, and each Mirror Wall tile, where a file dropped on the third tile goes to the third device whatever the device bar is pointed at. <b>Installing asks first and says what it is replacing</b>: the prompt shows the version it is moving from and to, names its button for what will actually happen (Install, Update, Reinstall, Downgrade &amp; Replace), and says up front when a downgrade cannot keep app data — and a package signed with a different key fails with an <b>Uninstall &amp; Install</b> button instead of a message you have to act on yourself. Copies report a real percentage and can be cancelled. <b>Nothing dropped on the app is ignored</b> any more: an APK on Reactotron opens the package screen, a video opens the Video Editor, anything else copies to the selected device, and files opened from Finder take the same route. <b>Pair over Wi-Fi by scanning a QR code</b> — show a code, point the phone’s Wireless debugging scanner at it, and pairing finishes with nothing typed: no pairing port, no six-digit code expiring while you type it. And <b>feeds cost less while you are not watching</b> — Logcat, the JS Console, Reactotron and iOS Logs pace themselves by visibility and by how busy the app is, so a hidden tab taking 300 log lines a second measured 8.7% CPU against 17.7% in v3.10.0. Plus the Wi-Fi pairing code no longer reaches the Command Log, and the mirror’s own window is now in the Window menu (⌃⌘M)." },
  { version: "v3.10.0", date: "Sep 2026", html: "<b>A quieter app, and a video editor that opens what ffmpeg reads</b> — <b>the Reactotron timeline was redrawing 62 times a second</b> whether or not anyone was looking at it, and because every open tab stays mounted that cost was paid across every feed you had open; with nine or more tabs it was the app's largest source of multi-second stalls. It now redraws four times a second while you're in the app and once a second while you're elsewhere, matching the JS Console and Logcat. <b>Failed HTTP requests are no longer reported as app errors</b> — Metro answers 500 to any stack frame it can't map, which is routine and already handled, and requests you send from API Testing were being captured the same way, with their URLs. <b>Reactotron reconnects after a restart</b>: restarting the app you're debugging now re-opens its <code>adb reverse</code> tunnel, and a tunnel that dies with the device still connected recovers instead of staying dead. <b>The video editor takes fifteen container formats</b> — mkv, webm, avi, flv, ts, m2ts, mpg, wmv, 3gp, ogv and more; anything macOS can't play is converted to a playable copy in the background while the export still runs from your original, and videos open from Finder via Open With. Plus <b>notifications while the app is in front</b> — anything reaching the in-app notification bar now also reaches Notification Center, and clicking one opens the bar on that entry — and <b>relaunching an emulator reports each stage</b> of the twenty-second wait it used to sit through silently. The Windows and Linux app is on the beta channel." },
  { version: "v3.9.2", date: "Aug 2026", html: "<b>Bug-fix and polish release</b> — <b>rows in the Reactotron timeline and the JS Console can be selected and copied whole</b>: <kbd>⌘</kbd>-click toggles a row, <kbd>⇧</kbd>-click takes the range from the last one picked, and dragging across rows sweeps a range, with the toolbar showing how many are picked and <kbd>⌘C</kbd> copying them. A copy carries the <i>whole</i> event rather than the line already on screen — the row’s summary followed by the complete payload, so an API call comes with its request body, headers and response, and an action or saga with its arguments; <b>Copy as JSON</b> gives the same events in the export’s wire form. Rows that were cleared, filtered out or trimmed while a selection stood are dropped from it, only a row’s header carries the drag (so dragging through an expanded payload still selects that text), and <kbd>⌘C</kbd> is bound only while rows are picked. <b>An API event’s status code now sits on its collapsed row</b> — green for 2xx, orange for a client error, red for a server error, neutral for a redirect, and <code>ERR</code> for a request that never got a response — so finding the one call that didn’t return 200 no longer means expanding rows one at a time. Plus a fix: <b>no empty “Screen Mirror” window at launch</b>, which in the worst case was the only window the app had; a pop-out mirror now has to have been asked for in this session." },
  { version: "v3.9.1", date: "Aug 2026", html: "<b>Bug-fix release</b> — <b>a pop-out mirror window no longer hangs the app</b>. The window re-registered itself on every view update and each registration wrote state the app's own window list reads, so the write ran the update and the update ran the write; the app beach-balled for as long as one of those windows was open, with no way out. Those windows are also no longer reopened from macOS’s saved state at launch, which had been starting a mirror session on a device nobody asked about — and made the hang survive reinstalling the app, since saved state lives outside the bundle. In <b>Reactotron</b>, <b>a stringified payload shows as an object</b>: an API request’s <code>data</code> field arrives as the app’s own <code>JSON.stringify</code> output, so an expanded event showed a wall of escaped text where the body should be — it now renders as a real tree with a raw toggle per row, and a logged string that is itself JSON gets the same. <b>Find agrees with what the tree shows</b> (searching <code>storeId</code> returned the whole escaped blob instead of the leaf below it), a <b>long API URL opens in place</b> instead of drawing over the rows beneath it, and a parsed payload stays with the tab it belongs to. Plus <b>Copy on the mirror capture sheet</b> — a screenshot taken on Mirror Screen or the Mirror Wall goes straight to the clipboard without opening the editor first — and a keystore password file that can’t be written now says why." },
  { version: "v3.9.0", date: "Aug 2026", html: "<b>The Mirror Wall</b> — the 61st tool mirrors up to <b>six connected devices side by side</b> in one pane, each tile live and controllable: click a tile and the tap goes to that device, type and the text does. Pick which devices the wall shows from its Devices menu (the set and the order are remembered per window), lay them out automatically or in 1, 2 or 3 fixed columns, and rearrange by dragging a tile by its caption strip. <b>Any tile can become its own window</b> — one device, or every tile at once — with <code>Arrange Mirror Windows</code> tiling them across the screen and Bring Back handing the device to the wall again; a pop-out mirror window is now pinned to its device instead of following the device bar, which is what lets several stand side by side. Six encoders cost real CPU, so the wall spends less per tile: the resolution and frame rate each device is asked for step down as tiles are added, tiles pause individually, audio comes from the focused tile only, and the wall stops after two minutes behind another tab, another window, or a minimised one. A device already mirrored elsewhere says so on its tile rather than getting a second encoder. <b>Full View (⇧⌘F)</b> hands the whole display to the screen you are on — sidebar, device bar and tab strip out of the way, full screen in. Plus fixes: <b>mirror sessions released their adb tunnel on quit</b> (one leaked per session, every quit — the single mirror included), and reconnecting a tile no longer leaves it drawing the stopped session's black frame." },
  { version: "v3.8.2", date: "Aug 2026", html: "<b>JS Console fixes for split panes</b> — the feed left blank space that grew every time the pane changed width, because a row measured its height against a baseline that was still moving and a wrapped message drew past the height its row reported. And the console <b>now shrinks to the pane it is given</b>: the connection bar held its target label at full width — <code>com.myapp.features · Pixel 7 - 17 - API 37</code> is most of a split pane — which became the minimum for everything below it, so rows were laid out wider than the pane and cut off at its edge. That also brings back the filter row's level, find, export and clear controls and each row's timestamp at the narrowest split. A <code>console.table</code> no longer sizes the whole feed, and <b>where a log came from is an icon</b> in the row's hover controls beside the copy buttons, with the file, line, function and path in its tooltip." },
