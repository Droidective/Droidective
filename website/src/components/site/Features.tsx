    ...
    <section id="features" className="mx-auto max-w-[1120px] px-6 py-26 max-[620px]:py-18">
      ...
      <div className="grid grid-cols-3 gap-4 max-[940px]:grid-cols-2 max-[620px]:grid-cols-1">
        {features.map((feature, i) => (
          <Reveal key={feature.title} delay={Math.min(i % 3, 2) * 80}>
            <div className="h-full min-w-0 rounded-2xl border border-border bg-linear-to-b from-white/2 to-white/0 p-6 pb-5.5 transition-[transform,border-color] duration-150 hover:-translate-y-[3px] hover:border-green/30 motion-reduce:hover:translate-y-0">
              ...
              <p className="mb-4 text-[14.5px] text-muted">{feature.body}</p>
