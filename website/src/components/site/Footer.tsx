...
export function Footer() {
  return (
    <footer className="border-t border-border px-6 pt-14 pb-16 text-sm text-muted">
      <div className="mx-auto max-w-[1120px]">
        <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr] gap-10 max-[940px]:grid-cols-2 max-[940px]:gap-8 max-[620px]:grid-cols-1">
          {/* Brand column */}
          <div>
            <a href="#top" className="mb-4 flex items-center gap-2.75 font-bold text-text">
              <img src="/assets/icon-light-64.png" alt="" width={26} height={26} loading="lazy" decoding="async" className="rounded-md" />
              Droidective
            </a>
            <p className="mt-3 max-w-[30ch] text-[13px] leading-[1.7] text-faint">
          ...
          {columns.map((col) => (
            <div key={col.title}>
              <p className="mb-3.5 font-mono text-[11px] tracking-[0.06em] text-faint uppercase">{col.title}</p>
              <ul className="m-0 list-none space-y-2.5 p-0">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="text-[13.5px] transition-colors duration-150 hover:text-green"
                    >
                      {link.label}
                    </a>
                  </li>
        ...
        <div className="mt-12 border-t border-border pt-6">
          <p className="max-w-[76ch] text-xs leading-[1.7] text-faint">
