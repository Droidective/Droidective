...
drag and drop, translucency, the role picker, a native menu bar), and 30 of the
...
Testing and APK Studio included. The two without one are `frida-console` (not
started; it needs a rooted device) and `ios-logs` (out of scope — a macOS
toolchain); the seven remaining catalog features are actions that render from
their registry fields. Still Mac-only: the welcome tour, the updater and Settings ▸ MCP — and
