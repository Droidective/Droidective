...
RELEASING.md is the authority (setup, secrets, beta channel); this is the
operational checklist. Work on a `release/vX.Y.Z` branch off latest
`origin/main`, one "Prepare vX.Y.Z release" commit, PR to main.
...
## 1. Version touchpoints — ALL of these, every release

| file | change |
|---|---|
| `project.yml` | `MARKETING_VERSION: "X.Y.Z"` |
| `RELEASE_NOTES.md` | new `## Droidective vX.Y.Z` section at the TOP (summary paragraph, themed `###` sections with bold leads, `### Install` last). CI extracts the first section for the GitHub release and the Sparkle appcast — plain factual language, no superlatives |
...
| `CLAUDE.md` | Status section: new "(Latest release: **vX.Y.Z** — …)" entry, previous release shifts into the "Before that" chain; test count if it moved |
| `README.md` / `docs/` | only if features or counts changed (registry total, marketing copy) |
| `site/sitemap.xml` | `<lastmod>` on `/` and `/changelog/` → release date |
...
- GitHub release: right version, notes rendered, DMG attached.
