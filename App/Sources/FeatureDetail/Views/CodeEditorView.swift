...
struct CodeEditorView: NSViewRepresentable {
    ...
    func updateNSView(_ webView: WKWebView, context: Context) {
        context.coordinator.load(content: content, language: language)
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ready = true
            if let (text, language) = pending {
                pending = nil
                run(text, language)
            }
        }

        func load(content: String, language: String) {
            guard loaded?.0 != content || loaded?.1 != language else { return }
            loaded = (content, language)
            if ready { run(content, language) } else { pending = (content, language) }
        }
        ...
        private func run(_ content: String, _ language: String) {
            guard let webView,
                  let data = try? JSONEncoder().encode(content),
                  let json = String(data: data, encoding: .utf8) else { return }
            webView.evaluateJavaScript("window.cmLoad(\(json), \"\(language)\")")
        }
    }
}
