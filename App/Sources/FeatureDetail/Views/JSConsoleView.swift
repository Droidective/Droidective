...
struct JSConsoleView: View {
    ...
    private func scrollingLog(_ visible: [JSEntry]) -> some View {
        feed(visible)
            ...
            // out, trimmed) would copy logs nobody can see.
    ...
    private func select(_ entry: JSEntry, in visible: [JSEntry], click: LogRowClick) {
        switch click {
        case .toggle: selection.toggle(entry.id)
        case .extend: selection.extend(to: entry.id, in: visible.map(\.id))
